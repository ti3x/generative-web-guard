...
var DEFAULT_LIMITS = Object.freeze({
  ...
  maxDiagnosticChars: 2e3,
  dataMs: 2e3
});
var JSON_ESCAPE_FACTOR = 6;
var TEMPLATE_OVERHEAD_CHARS = 1024;
var MAX_ALLOCATION_CHARS = 32 * 1024 * 1024;
function packedChars(chars) {
  return JSON_ESCAPE_FACTOR * chars + TEMPLATE_OVERHEAD_CHARS;
}
function stepProgramLimit(limits) {
  return packedChars(limits.maxStateChars + limits.maxEventChars);
}
function loadProgramLimit(limits) {
  return packedChars(limits.maxSourceChars + limits.maxDataChars);
}
function packetLimit(limits) {
...
var REQUEST_TYPES = Object.freeze(Object.keys(REQUEST_FIELDS));
var NULLABLE_FIELDS = Object.freeze(["data", "event"]);
...
function checkResult(type, result, limits = DEFAULT_LIMITS) {
  if (type === "load") {
    const shape2 = exactKeys(result, ["loaded"], "load result");
    if (shape2) return shape2;
  ...
  return checkBoundedString(result.state, limits.maxStateChars, `${type} result state`) || checkBoundedString(result.view, limits.maxViewChars, `${type} result view`);
}
function checkReply(msg, { id, type, limits = DEFAULT_LIMITS } = {}) {
  ...
  if (id === void 0 || type === void 0) return `unexpected reply ${msg.id} with no request outstanding`;
...
function errorText(err, maxChars = DEFAULT_LIMITS.maxDiagnosticChars) {
...
var POLICY_PROTOCOL_VERSION = 1;
var HOST_DOCUMENT_CODE_UNITS = 512e3;
var POLICY_MESSAGE = Object.freeze({
  // host -> worker
...
var POLICY_DELIVERY = Object.freeze({
...
var PREPROCESS_LIMITS = Object.freeze({
  // ---- source text, checked before parse5 is invoked --------------------
  maxSourceCodeUnits: Math.max(HOST_DOCUMENT_CODE_UNITS, DEFAULT_LIMITS.maxViewChars),
  ...
  maxRawAttrBytesUtf8PerElement: 64e3,
  ...
  maxRawTextCodeUnits: 2e5,
  maxRawTotalTextCodeUnits: 1e6,
  ...
  maxDiagnosticsUtf8Bytes: 16e3,
  maxDetailCodeUnits: 200
});
var POLICY_TIMEOUTS = Object.freeze({
  requestMs: 5e3,
  startupMs: 2e4
});
...
function policyRejection(code, fields = {}) {
  const reason = { code };
  for (const key of ["limit", "limitValue", "observed", "depth", "tag", "attr", "index"]) {
    if (fields[key] !== void 0) reason[key] = fields[key];
  }
...
function isPolicyEnvelope(message, expect = {}) {
  if (!message || typeof message !== "object") return false;
...
// src/adapters/parse5.js
var PreprocessLimitError = class extends Error {
  constructor(reason) {
...
function preprocessHtml(html, options = {}) {
  ...
  try {
    ...
    return policyRejection("parser-failed", { detail: errorText2(error) });
  }
  return convert(fragment, limits, now);
}
function parseHtmlToRaw(html, options) {
...
function convert(fragment, limits, now) {
  ...
  while (stack.length > 0) {
    ...
    const ns = node.namespaceURI === NS2.html ? "html" : node.namespaceURI === NS2.svg ? "svg" : "other";
    const children = [];
...
// src/startup.js
var STARTUP_STAGES = Object.freeze({
...
var STARTUP_TIMEOUTS = Object.freeze({
  workerCreateMs: 2e3,
  channelHandshakeMs: 5e3,
  wasmInitMs: 15e3,
  frameBootstrapMs: 5e3
});
var STARTUP_ERRORS = Object.freeze({
...
var STARTUP_WARNINGS = Object.freeze({
...
var StartupError = class extends Error {
  constructor(code, extra = {}) {
    ...
    this.stage = stage;
    this.hint = hint;
    if (extra.detail !== void 0) this.detail = String(extra.detail).slice(0, 300);
    if (extra.timeoutMs !== void 0) this.timeoutMs = extra.timeoutMs;
    if (extra.component !== void 0) this.component = extra.component;
  }
  ...
  toJSON() {
    return {
      ...
      ...this.component !== void 0 ? { component: this.component } : {}
    };
  }
};
...
function optionalInt(v) {
  return v === void 0 || Number.isInteger(v) && Math.abs(v) <= 1e6;
}
function sanitizeEvent(raw) {
  if (!raw || typeof raw !== "object") return null;
  const own = (k) => Object.hasOwn(raw, k) ? raw[k] : void 0;
  const e = {};
  ...
  for (const k of ["value", "dataValue", "dataKey", "key", "checked", "x", "y"]) {
    if (e[k] !== void 0) out[k] = e[k];
  }
  return out;
}
function buildFrameDocument(manifest) {
  const csp = [
    "default-src 'none'",
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="${csp}"><meta name="referrer" content="no-referrer"><meta name="color-scheme" content="light dark"><style>${manifest.css}</style></head><body><div id="root"></div><script>${manifest.script}<\/script></body></html>`;
}
function createSandboxFrame({
  ...
  const boundPromise = new Promise((resolve) => {
    settleBound = resolve;
  });
  ...
  function onMessage(e) {
    ...
    switch (msg.type) {
      ...
      case "bound": {
        ...
        if (bootstrapResolve) {
          const resolve = bootstrapResolve;
          bootstrapResolve = null;
          resolve(true);
        }
        if (settleBound) {
  ...
  const api = {
    ...
    attachPort(port, { instanceId, sessionId }) {
      ...
      return new Promise((resolve) => {
        if (bootstrapResolve) {
          bootstrapResolve(false);
          bootstrapResolve = null;
        }
        if (pendingBootstrap) {
          pendingBootstrap.port.close();
          pendingBootstrap.resolve(false);
          pendingBootstrap = null;
        }
        if (!ready) {
          pendingBootstrap = { message, port, resolve };
          return;
        }
        bootstrapResolve = resolve;
        if (!postBootstrap({ message, port })) {
          bootstrapResolve = null;
          resolve(false);
        }
      });
...
function createRuntimeController({
  ...
  function pump() {
    while (!dead && !inFlight && queue.length > 0) {
      ...
      request("step", { state: stateJson, event: eventJson }, watchdogMs).then((result) => {
  ...
  return {
    ...
    async load(source, data = void 0) {
      ...
      try {
        ...
        worker = null;
        const startupError = error instanceof StartupError ? error : new StartupError(
      ...
      worker.addEventListener("error", () => markDead(ready ? "worker error" : "worker error during startup (Firefox/WebKit report no message); check that the host policy's worker source directive contains blob:"));
      generation++;
...
var CAPABILITY_RULES = Object.freeze([RULES.CAP_INVENTORY, RULES.CAP_CEILINGS, RULES.CAP_CONTROLS]);
var CAPABILITIES = Object.freeze({
  ...
  "meaning": "Closed capability kernel for generated HTML/SVG output. Version 1 means: element and attribute identities listed here are the only ones any profile may permit; each attribute's descriptor is the widest value grammar reviewed for that attribute in that context; forcedAttributes and requiredAttributes are mandatory controls; ceilings are absolute upper bounds on the structural limits; excluded* names must never appear in accepted output. A profile may only restrict this kernel. Widening it (new element, new attribute, wider grammar, higher ceiling, removed requirement) is a kernel change: it needs security review of browser effects, renderer assumptions, tests and proof scope, and a capabilityVersion increment.",
  "reviewedAt": "2026-09-12",
  "reviewScope": "Reviewed against the raw-tree policy and the accepted-output theorems only. It says nothing about parser resource use, QuickJS isolation, host/frame messaging, CSS layout cost, or browser implementation bugs.",
  "families": {
    "plain": [
      "num",
      "nonNeg",
...
var LEAN_ABI_LIMITS = Object.freeze({
  ...
  maxResponseUtf8Bytes: 8e6
});
var LEAN_STATUS = Object.freeze({
  ok: 0,
  init: -1,
  length: -2,
  utf8: -3,
...
var ALLOWED_STATUS = Object.freeze({
  configure: /* @__PURE__ */ new Set(["configured", "error"]),
  info: /* @__PURE__ */ new Set(["info", "error"]),
...
function newId(prefix) {
  counter += 1;
  const random = globalThis.crypto && typeof globalThis.crypto.randomUUID === "function" ? globalThis.crypto.randomUUID() : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
...
function createPolicySession(options = {}) {
  ...
  function terminate(reason, timedOutRequestId = null) {
    ...
    if (!handshakeDone && failReady && !readySettled) {
      if (reason.code === "disposed") {
        ...
        failReady(new StartupError(
          reason.code === "channel-handshake-timeout" ? "channel-handshake-timeout" : classifyStartupFailure(STARTUP_STAGES.channelHandshake, reason.detail, "worker-startup-error"),
          { component: "policy-worker", detail: reason.detail, timeoutMs: timeouts.channelHandshakeMs }
        ));
      }
    }
    if (failChecker && !checkerReadySettled) {
      failChecker(reason.code === "disposed" ? new Error("policy session disposed before the checker was ready") : new StartupError(reason.code in STARTUP_ERRORS ? reason.code : "wasm-init-timeout", {
    ...
    for (const [requestId, entry] of [...pending.entries()]) {
      const body = requestId === timedOutRequestId ? { status: "rejected", reason: { code: reason.code, limit: "requestMs", limitValue: entry.budget } } : { status: "rejected", reason: { code: "session-terminated", detail: reason.code } };
      settle(entry, requestId, body);
    }
    if (onTerminated) {
      try {
        onTerminated({ ...reason, sessionId: dyingSession });
      } catch {
      }
    }
  }
  function onWorkerError(event) {
    ...
    if (!handshakeDone) {
      return terminate({
        ...
        stage: STARTUP_STAGES.workerCreate,
        detail: detail === "worker error" ? "opaque Worker error event during startup (Firefox/WebKit report no message); check that the host policy's worker source directive contains blob:" : detail
      });
    }
  ...
  function onMessage(event) {
    ...
    if (!isPolicyEnvelope(message, { instanceId, sessionId })) return;
    ...
    if (message.status === "rendered") {
      ...
      if (!isAcceptanceToken(token) || token.instanceId !== instanceId || token.sessionId !== sessionId || token.requestId !== message.requestId || token.generation !== message.generation || token.checkerVersion !== LEAN_CHECKER_VERSION) {
        stats.rejected += 1;
    ...
    if (message.status === "accepted") {
      ...
      const token = message.acceptance;
      if (token.instanceId !== instanceId || token.sessionId !== sessionId || token.requestId !== message.requestId || token.generation !== message.generation) {
        stats.rejected += 1;
        return settle(entry, message.requestId, { status: "rejected", reason: { code: "acceptance-identity-mismatch" } });
      }
  ...
  function preprocess(html, requestOptions = {}) {
    ...
    if (fresh) {
      try {
        ...
        const reason = error instanceof StartupError ? error.toJSON() : { code: "worker-create-failed", detail: String(error && error.message).slice(0, 200) };
        stats.requests += 1;
        stats.rejected += 1;
    ...
    return new Promise((resolve) => {
      ...
      entry.timer = setTimeout(() => {
        stats.timeouts += 1;
        const reason = entry.posted ? { code: "timeout", detail: `request ${requestId} exceeded ${budget}ms` } : { code: "frame-attach-timeout", detail: `the Worker did not confirm the frame port within ${budget}ms` };
