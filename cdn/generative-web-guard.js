...
function createSandboxFrame({ container, manifest, onEvent, onStatus, title = "Generated content" }) {
...
// src/gate.js
var DENIED_IDENTIFIERS = /* @__PURE__ */ new Set([
  "eval",
  "Function",
  "globalThis",
  "window",
...
function gateProgram(source) {
  const reasons = [];
  if (typeof source !== "string") return { status: "rejected", reasons: [{ code: "not-a-string" }] };
  if (source.length > 2e5) return { status: "rejected", reasons: [{ code: "too-long" }] };
  let ast;
  try {
...
function createRuntimeController({ createWorker, watchdogMs = 1500, loadWatchdogMs = 15e3, maxQueue = 32, onDead }) {
  let worker = null;
  let nextId = 1;
  let generation = 0;
  let stateJson = null;
  ...
  function onMessage(e) {
    const msg = e.data;
    if (!inFlight || !msg || msg.id !== inFlight.id) return;
    clearTimeout(inFlight.timer);
  ...
  function pump() {
    ...
    request("step", { state: stateJson, event: JSON.stringify(job.event) }).then((result) => {
  ...
  return {
    ...
    async load(source, data2 = void 0) {
      ...
      generation++;
      try {
        const dataJson = data2 === void 0 ? null : JSON.stringify(data2);
        if (data2 !== void 0 && typeof dataJson !== "string") throw new Error("data is not JSON-serializable");
