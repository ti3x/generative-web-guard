...
// src/gate.js
var UNSUPPORTED_IDENTIFIERS = /* @__PURE__ */ new Set([
  "eval",
  "Function",
  "globalThis",
  "window",
...
var REQUIRED = ["initialState", "update", "view"];
var LINT_BUDGETS = Object.freeze({
  maxSourceCodeUnits: 2e5,
  // UTF-16 code units
  maxAstNodes: 4e5,
  // AST nodes visited
  maxWalkMs: 250
...
function gateProgram(source, options = {}) {
  ...
  let ast;
  try {
    ast = parse3(source, { ecmaVersion: 2022, sourceType: "script", allowHashBang: false, locations: true });
  } catch (err) {
  ...
  const at2 = (node) => node.loc ? { line: node.loc.start.line, column: node.loc.start.column } : {};
  const reject = (node, code, message) => reasons.push({ code, message, ...at2(node) });
  const walked = walk(ast, (node, parent) => {
    switch (node.type) {
      ...
      case "ExportAllDeclaration":
        reject(node, "module-syntax", "imports and exports are not supported");
        break;
      case "AwaitExpression":
        reject(node, "async", "await is not supported");
        break;
      case "YieldExpression":
        reject(node, "generator", "generators are not supported");
        break;
      case "WithStatement":
        reject(node, "with", "with is not supported");
        break;
      case "DebuggerStatement":
        reject(node, "debugger", "debugger is not supported");
        break;
      case "MetaProperty":
        reject(node, "meta-property", "import.meta and new.target are not supported");
        break;
...
function walk(root, visit, budgets, now) {
  ...
  while (stack.length > 0) {
    ...
    if ((visited & 1023) === 0 && now() > deadline) return { ok: false, code: "time-budget", visited };
    visit(node, parent);
    children.length = 0;
